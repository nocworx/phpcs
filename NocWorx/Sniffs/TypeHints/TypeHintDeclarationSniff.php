...
use NocWorx\Helpers\Annotation;
use NocWorx\Helpers\AnnotationHelper;
use NocWorx\Helpers\DocCommentHelper;
use NocWorx\Helpers\FunctionHelper;
use NocWorx\Helpers\NamespaceHelper;
use NocWorx\Helpers\PropertyHelper;
use NocWorx\Helpers\ReturnTypeHint;
use NocWorx\Helpers\SniffSettingsHelper;
use NocWorx\Helpers\SuppressHelper;
use NocWorx\Helpers\TokenHelper;
use NocWorx\Helpers\TypeHintHelper;

class TypeHintDeclarationSniff implements \PHP_CodeSniffer\Sniffs\Sniff
{
	private const VALID_SCALARS = ['string', 'bool', 'int', 'array', 'float', 'integer', 'boolean' ];
	private const NAME = 'NocWorx.TypeHints.TypeHintDeclaration';
	 ...
	 * @var bool
	 */
	public $enableVoidTypeHint = false;
	...
	/** @var bool */
	public $ignoreMissingTraversablePropertyTypeHintSpecification = true;

	/** @var bool */
	public $ignoreMissingTraversableParameterTypeHintSpecification = true;

	/** @var bool */
	public $ignoreMissingTraversableReturnTypeHintSpecification = true;

	/** @var bool */
	public $ingoreMissingParameterTypeHint = true;

	/** @var bool */
	public $ignoreUselessDocComments = true;
	/**
	...
	public function process(\PHP_CodeSniffer\Files\File $phpcsFile, $pointer): void
	{
		...
		if ($token['code'] === T_FUNCTION) {
			...
			if (!$this->ignoreUselessDocComments) {
	...
	private function checkParametersTypeHints(\PHP_CodeSniffer\Files\File $phpcsFile, int $functionPointer): void
	{
		...
		if (! $this->ignoreMissingTraversableParameterTypeHintSpecification &&
			!SuppressHelper::isSniffSuppressed($phpcsFile, $functionPointer, $this->getSniffName(self::CODE_MISSING_TRAVERSABLE_PARAMETER_TYPE_HINT_SPECIFICATION))) {
		...
		foreach (FunctionHelper::getParametersWithoutTypeHint($phpcsFile, $functionPointer) as $parameterName) {
			...
			if (strpos($parameterTypeHintDefinition, "|")===false && $this->definitionContainsOneTypeHint($parameterTypeHintDefinition)) {
				...
				if (strpos($parameterTypeHintDefinition, "|")===false && $this->definitionContainsNullTypeHint($parameterTypeHintDefinition)) {
			...
			if (strpos($parameterTypeHintDefinition, "|")===false) {
				$phpcsFile->addWarning(
					sprintf(
						'%s %s() does not have parameter type hint for its parameter %s but it should be possible to add it based on @param annotation "%s".',
						$this->getFunctionTypeLabel($phpcsFile, $functionPointer),
	...
	private function checkReturnTypeHints(\PHP_CodeSniffer\Files\File $phpcsFile, int $functionPointer): void
	{
		...
		if (!$this->ignoreMissingTraversableReturnTypeHintSpecification &&
			!SuppressHelper::isSniffSuppressed($phpcsFile, $functionPointer, $this->getSniffName(self::CODE_MISSING_TRAVERSABLE_RETURN_TYPE_HINT_SPECIFICATION))) {
			if ($traversableTypeHint && !$hasReturnAnnotation) {
				...
				/** @var \NocWorx\Helpers\Annotation $returnAnnotation */
		...
		if (strpos($returnTypeHintDefinition, "|")===false && $this->enableVoidTypeHint && $returnTypeHintDefinition === 'void' && !$returnsValue && !array_key_exists(FunctionHelper::getName($phpcsFile, $functionPointer), $methodsWithoutVoidSupport)) {
			$phpcsFile->addWarning(
				sprintf(
		...
		if (strpos($returnTypeHintDefinition, "|")===false && $this->definitionContainsOneTypeHint($returnTypeHintDefinition)) {
		...
		} elseif (strpos($returnTypeHintDefinition, "|")===false && $this->definitionContainsJustTwoTypeHints($returnTypeHintDefinition)) {
		...
		if (strpos($returnTypeHintDefinition, "|")===false) {
	...
	private function checkPropertyTypeHint(\PHP_CodeSniffer\Files\File $phpcsFile, int $propertyPointer): void
	{
		...
		if (count($varAnnotations) === 0) {
			...
			if ($this->ignoreMissingTraversablePropertyTypeHintSpecification ||
				SuppressHelper::isSniffSuppressed($phpcsFile, $propertyPointer, $this->getSniffName(self::CODE_MISSING_TRAVERSABLE_PROPERTY_TYPE_HINT_SPECIFICATION))) {
				return;
			}
	 ...
	 * @param \PHP_CodeSniffer\Files\File $phpcsFile
	 * @param int $functionPointer
	 * @param \NocWorx\Helpers\ParameterTypeHint[]|null[] $functionTypeHints
